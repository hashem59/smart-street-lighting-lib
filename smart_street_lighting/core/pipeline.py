...
# When this module ran out of the playground repo, PROJECT_ROOT was the repo
# root; in an installed-package world we anchor on CWD instead so runtime caches
# (data/cache/, outputs/, etc.) land in the user's working directory.
PROJECT_ROOT = Path.cwd()

from smart_street_lighting.core.config import (
    LM_STUDIO_BASE_URL,
...
from smart_street_lighting.data.load_melbourne_data import (
    load_pedestrian_data,
    load_streetlight_data,
    get_sensor_summary,
)
from smart_street_lighting.data.spatial_analysis import (
    match_sensors_to_lights,
    analyze_lighting_efficiency,
    get_area_lighting_context,
)
from smart_street_lighting.data.temporal_analysis import (
...
from smart_street_lighting.llm.calculation_engine import design_lighting, format_design_report, P_CATEGORIES
...
class AppData:
    ...
    def __init__(self, ped_limit: int = 2000, light_limit: int = 5000):
        log.info("Loading Melbourne Open Data...")
        ...
        log.info("Running spatial analysis...")
...
def parse_query(user_message: str) -> dict:
    ...
    try:
        ...
        log.error(f"LLM parse failed ({e}), using regex fallback.")
...
def get_spatial_context(
    ...
    if bounds is None:
        log.warning(
            "No OSM boundary available for %r - skipping spatial context.",
            location,
        )
        return {
            "area_name": location,
...
def get_safety_context(location: str) -> dict | None:
    """Look up crime/safety risk for a location.

    Returns the safety score dict augmented with ``fallback_used`` and
    ``fallback_reason``. Two distinct fallback paths are exposed:

    1. The location is not in PARK_LGA_MAP -> LGA defaults to "Melbourne"
       (handled in :func:`data.safety_analysis.get_lga_with_fallback_info`).
    2. The LGA has no cached crime data -> safety score defaults to 5.0
       moderate (handled in :func:`data.safety_analysis.calculate_safety_score`).

    Both are surfaced via the ``fallback_used`` / ``fallback_reason`` fields
    on the returned dict. (S5-02 / item 11.)
    """
    try:
        ...
        lga_info = get_lga_with_fallback_info(location)
        result = calculate_safety_score(lga_info["lga_name"])
        # Merge the LGA-side fallback flag into the safety_context.
        # Crime-data fallback takes priority because it is the bigger
        # data-quality issue (the score is meaningless without the data).
        if lga_info["fallback_used"] and not result.get("fallback_used", False):
            result["fallback_used"] = True
            result["fallback_reason"] = lga_info["fallback_reason"]
        elif lga_info["fallback_used"]:
            result["fallback_reason"] = (
                result.get("fallback_reason", "") + " " + lga_info["fallback_reason"]
            ).strip()
        return result
    except Exception as e:
        log.error(f"Safety analysis failed for '{location}': {e}")
        return None
...
def resolve_osm_pathway(location: str) -> dict | None:
    ...
    """
    try:
        from smart_street_lighting.data.osm_loader import resolve_pathway

        return resolve_pathway(location)
    except Exception as e:
        log.error(f"OSM resolution failed for '{location}': {e}")
        return None
...
def run_design(
    ...
    *,
    ...
    log.info(
        "Parsed query parameters: location=%s, length=%sm, width=%sm, activity_level=%s, location_type=%s",
        location, length, width, activity, loc_type,
    )
    ...
    log.info(f"OSM data for '{location}': {json.dumps(osm_data, indent=2)}")
    ...
    log.info(
        "Spatial context for %s: %s streetlights, avg lux %s, avg pedestrian count %s/hr",
        location,
        spatial.get("num_streetlights", "N/A"),
        spatial.get("avg_lux_level", "N/A"),
        spatial.get("avg_pedestrian_count", "N/A"),
    )

    # print spatial data for debugging
    log.info(f"Spatial data for '{location}': {json.dumps(spatial, indent=2)}")
    ...
    sensor_fallback_used = False
    sensor_fallback_reason = ""
    if avg_traffic == 0:
        avg_traffic = data.sensor_summary["avg_hourly_traffic"].median()
        sensor_fallback_used = True
        sensor_fallback_reason = (
            f"No pedestrian sensor data is available within the project's "
            f"k-NN radius of {location!r}; the design uses the city-wide "
            f"median traffic ({avg_traffic:.0f} ped/hr) which may not "
            "reflect this specific corridor."
        )
    ...
    # Aggregate data-quality flags so the UI / LLM can surface them to the
    # user (S5-02 / item 11).
    data_quality = []
    if safety_context and safety_context.get("fallback_used"):
        data_quality.append({
            "field": "safety_risk_score",
            "fallback_reason": safety_context.get("fallback_reason", ""),
        })
    if sensor_fallback_used:
        data_quality.append({
            "field": "avg_pedestrian_count",
            "fallback_reason": sensor_fallback_reason,
        })
    design.data_quality = data_quality

...
# Anchor on CWD so downloaded CSVs land in the user's working directory
# (./data/cache/), not inside the installed-package location.
...
def fetch_melbourne_csv(dataset_name: str, limit: int = -1, use_cache: bool = True) -> pd.DataFrame:
    ...
    if use_cache and cache_file.exists():
        log.info(f"Loading cached {dataset_name}...")
    ...
    if limit > 0 and limit <= 500:
        ...
        while True:
            ...
            log.info(f"  Fetched {len(chunk)} rows (offset {offset})...")
            offset += page_size
    ...
    if use_cache and not df.empty:
        ...
        log.info(f"Cached {len(df)} rows to {cache_file}")
...
def load_pedestrian_data(limit: int = 5000) -> pd.DataFrame:
    ...
    log.info(f"Pedestrian data: {len(df)} rows, {df['sensor_name'].nunique()} unique sensors")
    return df
...
def load_streetlight_data(limit: int = -1) -> pd.DataFrame:
    ...
    log.info(f"Streetlight data: {len(df)} rows")
    return df

...
from smart_street_lighting.data.geometry import measure_polyline_length, find_intersections, find_entry_points

# OSM cache anchored on CWD so it persists in the user's project, not site-packages.
...
def geocode_nominatim(place_name: str, city: str = "Melbourne") -> Optional[dict]:
    ...
    try:
        ...
        log.error(f"Nominatim geocoding failed for '{place_name}': {e}")
        return None

    if not results:
        log.info(f"Nominatim found no results for '{place_name}' in {city}.")
        return None
...
def fetch_park_data(park_name: str, city: str = "Melbourne") -> Optional[dict]:
    ...
    try:
        ...
        log.error(f"Overpass query failed for park '{park_name}': {e}")
        return None

    elements = data.get("elements", [])
    if not elements:
        log.info(f"No OSM park found for '{park_name}' in {city}.")
        return None
    ...
    if not boundary_coords:
        log.info(f"Could not extract boundary for '{park_name}'.")
        return None
...
def fetch_pathways(park_boundary: dict) -> list[dict]:
    ...
    try:
        ...
        log.error(f"Overpass pathway query failed: {e}")
        return []
...
def resolve_pathway(
    ...
    if not park:
        ...
        log.info(f"Overpass unavailable for '{park_name}', trying Nominatim geocoding...")
        nominatim = geocode_nominatim(park_name)
        if nominatim:
            log.info(f"Nominatim resolved '{park_name}' (boundary only, no pathways).")
            result = {
    ...
    if not pathways_raw:
        log.info(f"No pathways found in '{park_name}'.")

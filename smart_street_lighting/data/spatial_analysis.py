...
def match_sensors_to_lights(
    ...
    if ped_clean.empty or light_clean.empty:
        log.warning("Warning: No valid coordinates for matching.")
        return ped_df
    ...
    log.info(f"Matched {len(ped_clean)} sensors to nearest streetlights.")
    log.info(f"  Avg distance to nearest light: {ped_clean['distance_to_light_m'].mean():.1f}m")
    log.info(f"  Max distance to nearest light: {ped_clean['distance_to_light_m'].max():.1f}m")
...
def analyze_lighting_efficiency(
    ...
    if lux_col not in ped_df.columns or traffic_col not in ped_df.columns:
        log.warning("Warning: Missing columns for efficiency analysis.")
        return ped_df

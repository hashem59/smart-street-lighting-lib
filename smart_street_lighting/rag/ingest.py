...
from smart_street_lighting.rag.lm_studio import LMStudioEmbedding

# Paths — anchored on the caller's CWD, not the installed-package location.
# The notebook is expected to download/extract the KB into ./data/knowledge_base/
# before calling ingest_knowledge_base(); ChromaDB persists to ./data/chroma_db/.
...
def ingest_knowledge_base(
    ...
    for source_dir in [KNOWLEDGE_BASE_DIR, DOWNLOADED_SOURCES_DIR]:
        if source_dir.exists():
            log.info(f"Loading documents from {source_dir}...")
            ...
            log.info(f"  Loaded {len(docs)} files from {source_dir.name}")
    ...
    log.info("Chunking and embedding documents (using local LM Studio)...")
    ...
    log.info(f"Ingestion complete. {num_chunks} chunks stored in ChromaDB.")
    return index
...
def load_existing_index(
    ...
    log.info(f"Loaded existing index with {chroma_collection.count()} chunks.")
    return index

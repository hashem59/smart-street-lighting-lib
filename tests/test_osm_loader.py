...
import pytest
from smart_street_lighting.data.geometry import (
...
class TestResolvePathway:
    def test_resolve_from_fixture(self):
        ...
        with patch("smart_street_lighting.data.osm_loader._is_cache_fresh", return_value=True), \
             patch("smart_street_lighting.data.osm_loader._load_cache", return_value=fixture):
    ...
    def test_resolve_returns_none_when_both_services_fail(self):
        ...
        with patch("smart_street_lighting.data.osm_loader._is_cache_fresh", return_value=False), \
             patch("smart_street_lighting.data.osm_loader._query_overpass", return_value={"elements": []}), \
             patch("smart_street_lighting.data.osm_loader.geocode_nominatim", return_value=None):
    ...
    def test_resolve_falls_back_to_nominatim(self):
        ...
        with patch("smart_street_lighting.data.osm_loader._is_cache_fresh", return_value=False), \
             patch("smart_street_lighting.data.osm_loader._query_overpass", return_value={"elements": []}), \
             patch("smart_street_lighting.data.osm_loader.geocode_nominatim", return_value=nominatim_result):
            result = resolve_pathway("Fitzroy Gardens")
...
class TestGeocodeNominatim:
    ...
    def test_returns_boundary_from_bbox(self):
        ...
        with patch("smart_street_lighting.data.osm_loader._is_cache_fresh", return_value=False), \
             patch("smart_street_lighting.data.osm_loader.requests.get", return_value=self._mock_nominatim_response(bbox)):
    ...
    def test_returns_none_when_no_results(self):
        ...
        mock_resp.json.return_value = []
        with patch("smart_street_lighting.data.osm_loader._is_cache_fresh", return_value=False), \
             patch("smart_street_lighting.data.osm_loader.requests.get", return_value=mock_resp):
    ...
    def test_returns_none_on_network_error(self):
        with patch("smart_street_lighting.data.osm_loader._is_cache_fresh", return_value=False), \
             patch("smart_street_lighting.data.osm_loader.requests.get", side_effect=ConnectionError("timeout")):
            result = geocode_nominatim("Fitzroy Gardens")
    ...
    def test_uses_cache_when_fresh(self):
        ...
        with patch("smart_street_lighting.data.osm_loader._is_cache_fresh", return_value=True), \
             patch("smart_street_lighting.data.osm_loader._load_cache", return_value=cached):
            result = geocode_nominatim("Fitzroy Gardens")

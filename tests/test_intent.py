...
import pytest
from smart_street_lighting.core.intent import (
    classify_intent,
...
class TestClassifyIntentLLM:
    ...
    def test_llm_classifies_new_design(self):
        with patch("smart_street_lighting.core.intent.requests.post", return_value=self._mock_llm_response("new_design")):
            result = classify_intent_llm("Design lighting for a new park on the south side")
        assert result["intent"] == "new_design"
        assert result["confidence"] == 0.90

    def test_llm_classifies_modify(self):
        with patch("smart_street_lighting.core.intent.requests.post", return_value=self._mock_llm_response("modify", "brightness")):
            result = classify_intent_llm("can we get more light near the benches")
        assert result["intent"] == "modify"
        assert result["modify_target"] == "brightness"

    def test_llm_classifies_question(self):
        with patch("smart_street_lighting.core.intent.requests.post", return_value=self._mock_llm_response("question")):
    ...
    def test_llm_sanitises_invalid_intent(self):
        ...
        with patch("smart_street_lighting.core.intent.requests.post", return_value=self._mock_llm_response("unknown_intent")):
...
class TestClassifyIntent:
    ...
    def test_falls_back_to_regex_when_llm_unavailable(self):
        ...
        with patch("smart_street_lighting.core.intent.classify_intent_llm", side_effect=ConnectionError("LM Studio down")):
    ...
    def test_uses_llm_when_available(self):
        ...
        with patch("smart_street_lighting.core.intent.classify_intent_llm", return_value=llm_result):
